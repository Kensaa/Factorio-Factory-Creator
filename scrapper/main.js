...
async function main(){
    ...
    for(let link of links){
        ...
        let craft = await page.evaluate(()=>{
            ...
            let infoLines = $(pb).find('tr').toArray();
            function find(f,infoLines){
                let pbindex = null;
                let i = 0;
                for(let s of infoLines){
                    if(s.className == "border-top"){
                        let title = $($(s).find('td')[0]).find('p').text().trim();
                        if(title == f)pbindex = i;
                    }
                    i++;
                }
                return pbindex;
            }
            let pbindex = find('Produced by',infoLines);
            let machineName = 'None';
            if(pbindex){
                let machines = $(infoLines[pbindex+1]).find('td > div').toArray();
                let firstMachine = machines.shift();
                machineName = $(firstMachine).find('a')[0].attributes.title.value.trim().replace('1',"").replace("Stone","").trim();
            }

            let typeIndex = find('Prototype type',infoLines);
            let type = 'None';
            if(typeIndex){
                type = $($(infoLines[typeIndex]).find('td')[1]).find('p > a').text();
            }

            let obj = {...finalItem,time,needed:infos,type,producedIn:machineName};
            console.log(obj);
            return obj;
        });
